...
export default function PlayerScreen({ player, channels }) {
  ...
  return (
    <View style={player.isFullscreen ? styles.videoContainerFullscreen : styles.videoContainer}>
      ...
      {/* ── VIDEO COMPONENT (hanya jika URL ada dan belum semua gagal) ── */}
      {player.currentUrl !== '' && !player.allUrlsFailed && (
        <Video
      ...
      {/* ── FALLBACK ERROR (ketika semua URL gagal) ── */}
      {player.allUrlsFailed && (
        <View style={styles.fallbackContainer}>
          <MaterialIcons name="error-outline" size={64} color="#ff6666" />
          <Text style={styles.fallbackText}>
            {'Stream gagal diputar.\nLisensi DRM mungkin kadaluarsa atau tidak valid.'}
          </Text>
          <TouchableOpacity
            style={styles.retryButton}
            onPress={() => player.changeChannel(currentChannelInfo)}
          >
      ...
      {/* ── LOADING OVERLAY (hanya jika sedang memuat) ── */}
      {player.isVideoLoading && !player.allUrlsFailed && (
...
const styles = StyleSheet.create({
  ...
  // Fallback ketika error total
  fallbackContainer: {
    justifyContent: 'center', alignItems: 'center', padding: 20,
  },
  fallbackText: {
    color: '#fff', fontSize: 14, textAlign: 'center', marginVertical: 12, lineHeight: 20,
  },
  retryButton: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#e53935',
    paddingHorizontal: 20, paddingVertical: 10, borderRadius: 25, marginTop: 10,
  },
  retryText: {
    color: '#fff', fontWeight: 'bold', marginLeft: 8,
  },
